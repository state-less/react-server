    ...
    try {
      ...
      const onMessage = async (data) => {
        ...
        if (action === ACTION_RENDER) {
          ...
          try {
            const res = await render(comp, props, {
              ...connectionInfo,
              headers,
            });
            ...
            if (phase === "response") {
              ...
              if (!strategy || !strategies[strategy]) {
                ...
                if (recoveredToken.compound) {
                  const mfaState = await store
                    .scope("public.mfa")
                    .scope(recoveredToken.compound.id)
                    .useState("2fa");
                ...
                const token = jwt.sign(
                  {
                    ...
                    factors: Object.keys(solvedFactors).filter(
                      (f) => !solvedFactors[f]
                    ),
                  },
                  secret
                ...
                if (!Object.values(solvedFactors).reduce((a, b) => a && b)) {
                  crypto.randomBytes(8, function (err, buffer) {
                    ...
                    socket.send(
                      success(
                        `Please sign this message to prove your identity: ${rand}`,
                        {
                          action: "auth",
                          phase: "challenge",
                          routeKey: "auth",
        ...
        if (action === ACTION_CALL) {
          ...
          const action = res.props.children.find(
            (action) => action?.props?.name === name
          );
          ...
          if (!action) {
            ...
            throw new Error(
              "No handler ${handler} defined for action ${action}"
            );
          }
          ...
          try {
            if (
              action.props.boundHandler.use &&
              typeof action.props.boundHandler.use === "function"
            ) {
        ...
        if (action === ACTION_SET_STATE) {
          const { action, key, scope, requestId, props, options, value, id } =
            json;
          const handler = ConnectionHandler(broker, store, "USE_STATE");
          const state: State = await handler(connectionInfo, {
            key: key ? key : id,
            scope,
            requestId,
            props,
