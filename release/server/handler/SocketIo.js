...
const handleSetState = (broker, store, socket) => (data, options = {}) => {
  ...
  const {
    useState,
    has
  } = createScope(store, scopeKey);
  if (!key || !has(key)) return console.error(`Error setting state with key ${key}`);
  const {
    setValue
  } = useState(key);
  setValue(value);
  ...
  socket.on('useState', (key, def, options = {}) => {
    ...
    if (permitted) {
      ...
      if (!socketSet.has(state)) {
        ...
        logger.debug`Syncing state ${state} with socket ${socket}`; //NOTE: echoing data might pose security risks maybe use configurable whitelists
...
ee(SocketIoHandler.prototype);
module.exports = {
  ConnectionHandler: SocketIoHandler
};