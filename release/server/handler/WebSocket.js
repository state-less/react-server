...
const ConnectionHandler = (broker, store, eventType) => {
  if (eventType === "USE_STATE") {
    return async (connectionInfo, data = {}) => {
      ...
      const scopedStore = getScopedStore(broker, store, data, connectionInfo); // logger.warning`Scoped store ${store}`;
      ...
      if (eventType === "DISCONNECT") {
        ...
        if (requestType === "request") await state.publish(broker, { ...connectionInfo,
          requestId,
          requestType
        });
      }
  ...
  return async (connectionInfo, data = {}) => {
    ...
    const scopedStore = getScopedStore(broker, store, options, connectionInfo); // logger.warning`Scoped store ${store}`;

    const authorized = getAuthorization(scopedStore, key, options, connectionInfo);
    const state = await useAsyncState(scopedStore, key, defaultValue, { ...options,
      connectionInfo
    });
